...
void zcom::CursorTrailScene::Init(SceneOptionsBase* options)
{
    CursorTrailSceneOptions opt;
    if (options)
        opt = *reinterpret_cast<const CursorTrailSceneOptions*>(options);
    ...
    _basePanel->SetBorderColor(D2D1::ColorF(0xFF0000));
    ...
    //_inputMessageSubscription = _app->GetMessageWindow()->SubscribeToWindowMessages([=](zwnd::WindowMessage msg) {
    //    if (msg.id == zwnd::MouseInputMessage::ID())
    //    {
    //        zwnd::MouseInputMessage message;
    //        message.Decode(msg);

    //        std::lock_guard<std::mutex> lock(_m_input);
    //        _currentMousePos.x += message.deltaX;
    //        _currentMousePos.y += message.deltaY;
    //        _inputClock.Update();
    //        zutil::ColorF newColor = zutil::ColorF::FromHSV(std::fmodf(_inputClock.Now().GetTime(MICROSECONDS) / (float)_colorCycleDuration.GetDuration(MICROSECONDS), 1.0f) * 360.0f, 100.0f, 100.0f);
    //        _pendingInput.push_back({
    //            Pos2D<float>(float(_currentMousePos.x), float(_currentMousePos.y)),
    //            _inputClock.Now(),
    //            D2D1::ColorF(newColor.r, newColor.g, newColor.b)
    //        });
    //    }
    //    return false;
    //});
    ...
    _headIconBitmap = _window->resourceManager.GetImage("trail_head_icon");
}

void zcom::CursorTrailScene::_UpdateParameters()
{
    _headColor = zutil::Color(_app->config.GetIntConfigValue(CursorTrailConfig::HEAD_COLOR, Config::ADD_AND_SAVE_IF_MISSING));
    _trailWidth = _app->config.GetIntConfigValue(CursorTrailConfig::TRAIL_WIDTH, Config::ADD_AND_SAVE_IF_MISSING);
    _headSize = _app->config.GetIntConfigValue(CursorTrailConfig::HEAD_SIZE, Config::ADD_AND_SAVE_IF_MISSING);
    _trailEdgeWidth = _app->config.GetDoubleConfigValue(CursorTrailConfig::TRAIL_EDGE_WIDTH, Config::ADD_AND_SAVE_IF_MISSING);
    _headEdgeWidth = _app->config.GetDoubleConfigValue(CursorTrailConfig::HEAD_EDGE_WIDTH, Config::ADD_AND_SAVE_IF_MISSING);
    _trailDuration = Duration(_app->config.GetIntConfigValue(CursorTrailConfig::TRAIL_LIFETIME, Config::ADD_AND_SAVE_IF_MISSING), MILLISECONDS);
    _colorCycleDuration = Duration(_app->config.GetIntConfigValue(CursorTrailConfig::COLOR_CYCLE_DURATION, Config::ADD_AND_SAVE_IF_MISSING), MILLISECONDS);
    _iconSpinDuration = Duration(_app->config.GetIntConfigValue(CursorTrailConfig::ICON_SPIN_DURATION, Config::ADD_AND_SAVE_IF_MISSING), MILLISECONDS);
    _trailResolution = _app->config.GetIntConfigValue(CursorTrailConfig::TRAIL_RESOLUTION, Config::ADD_AND_SAVE_IF_MISSING);

    _ParsePaletteString(_app->config.GetConfigValue(CursorTrailConfig::PALETTE, Config::ADD_AND_SAVE_IF_MISSING));
    if (_palette.empty())
    {
        _ParsePaletteString(CursorTrailConfig::PALETTE.defaultValue);
        _app->config.SetValue(CursorTrailConfig::PALETTE.name, CursorTrailConfig::PALETTE.defaultValue);
    }
}

void zcom::CursorTrailScene::_ParsePaletteString(const std::wstring& str)
{
    _palette.clear();
    ...
    for (auto& stopString : stopStrings)
    {
        ...
        _palette.push_back(std::move(GradientStop{ zutil::Color(std::stoi(parts[0])), std::stof(parts[1]) }));
    }
...
void zcom::CursorTrailScene::_Update()
{
    ...
    POINT p;
    GetCursorPos(&p);
    Pos2D<float> newPoint = Pos2D{ float(p.x), float(p.y) };
    if (!_points.empty())
    {
...
void zcom::CursorTrailScene::_Draw(Component* panel, Graphics g)
{
    ...
    g.target->CreateBitmap(D2D1::SizeU((UINT32)g.target->GetSize().width, (UINT32)g.target->GetSize().height), nullptr, 0, props, bitmap.GetAddressOf());

    ID2D1Effect* trailEffect;
    HRESULT hr = g.target->CreateEffect(CLSID_CursorTrailEffect, &trailEffect);
    if (trailEffect)
    {
        ...
        g.target->DrawImage(trailEffect, D2D1_INTERPOLATION_MODE_CUBIC);
        trailEffect->Release();
    }
    ...
    if (!_points.empty() && _headIconBitmap)
    {
        ...
        if (rotationEffect)
        {
            ...
            g.target->DrawImage(rotationEffect, D2D1::Point2F(_points.back().position.x - bitmapSize.width / 2.0f, _points.back().position.y - bitmapSize.height / 2.0f), D2D1_INTERPOLATION_MODE_HIGH_QUALITY_CUBIC);
            rotationEffect->Release();
        }
    }
}

D2D1_COLOR_F zcom::CursorTrailScene::_GetColorByTime(TimePoint time)
{
    if (_palette.empty())
    {
    ...
    else
    {
        ...
        zutil::Color color1 = _palette[index - 1].color;
        zutil::Color color2 = _palette[index].color;
