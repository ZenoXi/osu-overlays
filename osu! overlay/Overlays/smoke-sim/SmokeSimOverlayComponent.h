...
namespace zcom
{
    class SmokeSimOverlayComponent : public Panel
    {
        DEFINE_COMPONENT(SmokeSimOverlayComponent, Panel)
    public:
        ~SmokeSimOverlayComponent();
    protected:
        void Init(std::shared_ptr<const Overlay> overlay, SmokeSimType type);
    ...
    private:
        ...
        std::mutex _mtx;
        std::atomic_bool _stopSimulation = false;
        void _Simulate();
        ...
        std::vector<float> dens_to_render;
        std::vector<float> temp_to_render;
