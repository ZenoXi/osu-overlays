...
void zcom::ColorSelectorScene::Init(SceneOptionsBase* options)
{
    ...
    _initialColor = zutil::Color(_app->config.GetIntConfigValue(_configValue, Config::ADD_AND_SAVE_IF_MISSING));
    ...
    redInput->SubscribeOnValueChanged([=](NumberInputValue value) {
        _redSlider->SetValue(value.getAsInteger() / 255.0f);
    }).Detach();
    _redSlider->SubscribeOnValueChanged([=, redInput = redInput.get()](Slider*, float* newValue) {
        int redValue = int((*newValue) * 255);
        redInput->SetValue(NumberInputValue(redValue));
        _currentColor.r = redValue;
        _OnColorChanged();
    }).Detach();
    _redSlider->SubscribeOnEnterInteractionArea([=](Slider* slider) {
        slider->GetAnchorComponent()->SetBackgroundImage(_window->resourceManager.GetImage("slider_anchor_hovered"));
    }).Detach();
    _redSlider->SubscribeOnLeaveInteractionArea([=](Slider* slider) {
        slider->GetAnchorComponent()->SetBackgroundImage(_window->resourceManager.GetImage("slider_anchor"));
    }).Detach();
    ...
    greenInput->SubscribeOnValueChanged([=](NumberInputValue value) {
        _greenSlider->SetValue(value.getAsInteger() / 255.0f);
    }).Detach();
    _greenSlider->SubscribeOnValueChanged([=, greenInput = greenInput.get()](Slider*, float* newValue) {
        int greenValue = int((*newValue) * 255);
        greenInput->SetValue(NumberInputValue(greenValue));
        _currentColor.g = greenValue;
        _OnColorChanged();
    }).Detach();
    _greenSlider->SubscribeOnEnterInteractionArea([=](Slider* slider) {
        slider->GetAnchorComponent()->SetBackgroundImage(_window->resourceManager.GetImage("slider_anchor_hovered"));
    }).Detach();
    _greenSlider->SubscribeOnLeaveInteractionArea([=](Slider* slider) {
        slider->GetAnchorComponent()->SetBackgroundImage(_window->resourceManager.GetImage("slider_anchor"));
    }).Detach();
    ...
    blueInput->SubscribeOnValueChanged([=](NumberInputValue value) {
        _blueSlider->SetValue(value.getAsInteger() / 255.0f);
    }).Detach();
    _blueSlider->SubscribeOnValueChanged([=, blueInput = blueInput.get()](Slider*, float* newValue) {
        int blueValue = int((*newValue) * 255);
        blueInput->SetValue(NumberInputValue(blueValue));
        _currentColor.b = blueValue;
        _OnColorChanged();
    }).Detach();
    _blueSlider->SubscribeOnEnterInteractionArea([=](Slider* slider) {
        slider->GetAnchorComponent()->SetBackgroundImage(_window->resourceManager.GetImage("slider_anchor_hovered"));
    }).Detach();
    _blueSlider->SubscribeOnLeaveInteractionArea([=](Slider* slider) {
        slider->GetAnchorComponent()->SetBackgroundImage(_window->resourceManager.GetImage("slider_anchor"));
    }).Detach();
    ...
    opacityInput->SubscribeOnValueChanged([=](NumberInputValue value) {
        _opacitySlider->SetValue(value.getAsInteger() / 255.0f);
    }).Detach();
    _opacitySlider->SubscribeOnValueChanged([=, opacityInput = opacityInput.get()](Slider*, float* newValue) {
        int opacityValue = int((*newValue) * 255);
        opacityInput->SetValue(NumberInputValue(opacityValue));
        _currentColor.a = opacityValue;
        _OnColorChanged();
    }).Detach();
    _opacitySlider->SubscribeOnEnterInteractionArea([=](Slider* slider) {
        slider->GetAnchorComponent()->SetBackgroundImage(_window->resourceManager.GetImage("slider_anchor_hovered"));
    }).Detach();
    _opacitySlider->SubscribeOnLeaveInteractionArea([=](Slider* slider) {
        slider->GetAnchorComponent()->SetBackgroundImage(_window->resourceManager.GetImage("slider_anchor"));
    }).Detach();
    ...
    buttonRow->SetParentWidthPercent(1.0f);
    buttonRow->SetBaseHeight(26);
    buttonRow->SetProperty(PROP_Shadow());
    auto resetToDefaultButton = Create<Button>(L"Reset to default");
    resetToDefaultButton->SetBaseSize(120, 26);
    NeutralButtonStyle::Apply(resetToDefaultButton.get());
    resetToDefaultButton->RemoveProperty<PROP_Shadow>();
    resetToDefaultButton->SubscribeOnActivated([=] {
        _currentColor = zutil::Color(_configValue.defaultValue);
    ...
    revertButton->RemoveProperty<PROP_Shadow>();
    ...
    if (pGradientStops)
    {
        g.target->CreateLinearGradientBrush(
            D2D1::LinearGradientBrushProperties(
                D2D1::Point2F(0, item->GetHeight() / 2.0f),
                D2D1::Point2F((FLOAT)item->GetWidth(), item->GetHeight() / 2.0f)),
            pGradientStops,
            &pGradientBrush
        );
        if (pGradientBrush)
        {
            g.target->FillRectangle(D2D1::RectF(0, 0, (FLOAT)item->GetWidth(), (FLOAT)item->GetHeight()), pGradientBrush);
    ...
    if (patternCommandList)
    {
        ...
        if (cellBrush)
        {
            ...
            if (patternBrush)
            {
                ...
                g.target->FillRectangle(D2D1::RectF(0, 0, (FLOAT)item->GetWidth(), (FLOAT)item->GetHeight()), patternBrush);
