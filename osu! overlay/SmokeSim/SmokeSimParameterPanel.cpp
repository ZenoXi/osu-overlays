...
void zcom::SmokeSimParameterPanel::Init(SmokeSimType simType)
{
    ...
    simParams.trailColor = _scene->GetApp()->options.GetIntValue(L"smokesim.cursortrail.trailColor").value_or(simParams.trailColor.Default());
    ...
    simParams.smokeColor = _scene->GetApp()->options.GetIntValue(L"smokesim.enhancedsmoke.smokeColor").value_or(simParams.smokeColor.Default());
    ...
    simParams.smokeKeyCode = _scene->GetApp()->options.GetIntValue(L"smokesim.enhancedsmoke.smokeKeyCode").value_or(simParams.smokeKeyCode.Default());
    _scene->GetApp()->options.SetIntValue(L"smokesim.cursortrail.trailColor", simParams.trailColor.Get(), false);
    ...
    _scene->GetApp()->options.SetIntValue(L"smokesim.enhancedsmoke.smokeColor", simParams.smokeColor.Get(), false);
    ...
    _scene->GetApp()->options.SetIntValue(L"smokesim.enhancedsmoke.smokeKeyCode", simParams.smokeKeyCode.Get(), false);
    _scene->GetApp()->options.SaveOptions();

    if (simType == SmokeSimType::ENHANCED_SMOKE)
    {
        Handle<zwnd::Window> overlayWindowHandle = _scene->GetApp()->FindWindowByClassName(L"enhancedSmokeOverlay");
        if (overlayWindowHandle.Valid())
            _overlayWindowId = overlayWindowHandle->GetWindowId();
        Handle<zwnd::Window> colorSelectorWindowHandle = _scene->GetApp()->FindWindowByClassName(L"enhancedSmokeColorSelector");
    ...
    {
        Handle<zwnd::Window> overlayWindowHandle = _scene->GetApp()->FindWindowByClassName(L"cursorTrailOverlay");
        if (overlayWindowHandle.Valid())
            _overlayWindowId = overlayWindowHandle->GetWindowId();
        Handle<zwnd::Window> colorSelectorWindowHandle = _scene->GetApp()->FindWindowByClassName(L"cursorTrailColorSelector");
        if (colorSelectorWindowHandle.Valid())
            _colorSelectorWindowId = colorSelectorWindowHandle->GetWindowId();
    }
        ...
        _cellSizeInput->AddOnValueChanged([=](NumberInputValue value) {
            ...
            _threadCountInput->AddOnValueChanged([=](NumberInputValue value) {
                ...
                _fullMonitorCheckbox->SubscribeOnStateChanged([=](bool state) {
                    ...
                    auto keySelectorKeyCodeLabel = Create<Label>(KeySelector::KeyCodeNameMap.at(simParams.smokeKeyCode.Get()));
                    keySelectorKeyCodeLabel->SetBaseSize(80, 26);
                    keySelectorKeyCodeLabel->SetMargins({ 5.0f, 0.0f, 0.0f, 0.0f });
                    ...
                    auto keySelectorInput = Create<KeySelector>((BYTE)simParams.smokeKeyCode.Get());
                    ...
                    keySelectorInput->SubscribeOnKeySelected([=](BYTE value) {
                        _scene->GetApp()->options.SetIntValue(L"smokesim.enhancedsmoke.smokeKeyCode", value);
                        }).Detach();
                        keySelectorInput->SubscribeOnSelected([=, keySelectorKeyCodeLabel = keySelectorKeyCodeLabel.get()](Component*, bool) {
                            ...
                            keySelectorInput->SubscribeOnDeselected([=, keySelectorKeyCodeLabel = keySelectorKeyCodeLabel.get()](Component* item) {
                                ...
                                slowdownPersistenceDurationInput->AddOnValueChanged([=](NumberInputValue value) {
                                    ...
                                    appearanceLabel->SetMargins({ 15.0f, 0.0f, 15.0f, 0.0f });
...
void zcom::SmokeSimParameterPanel::_OnUpdate()
{
    if (_colorInput && _lastColorInputUpdate + Duration(250, MILLISECONDS) < ztime::Main())
    {
        _UpdateColorInput();
        _lastColorInputUpdate = ztime::Main();
    }
...
{
    std::optional<int> value = _scene->GetApp()->options.GetIntValue(_simType == SmokeSimType::CURSOR_TRAIL ? L"smokesim.cursortrail.trailColor" : L"smokesim.enhancedsmoke.smokeColor");
    zutil::Color color = zutil::Color(value ? value.value() : 0xFF000000);
...
{
    std::wstring wndClass = _simType == SmokeSimType::CURSOR_TRAIL ? L"cursorTrailColorSelector" : L"enhancedSmokeColorSelector";
    std::wstring windowTitle = _simType == SmokeSimType::CURSOR_TRAIL ? L"Trail color" : L"Smoke color";
    std::wstring optionName = _simType == SmokeSimType::CURSOR_TRAIL ? L"smokesim.cursortrail.trailColor" : L"smokesim.enhancedsmoke.smokeColor";
    int width = 300;
    int height = 214;
