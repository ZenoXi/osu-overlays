...
namespace zcom
{
    ...
    public:
        ...
        Duration ScrollFocusLifetime() const;
        ...
        void ScrollFocusLifetime(Duration lifetime);
    ...
    private:
        ...
        Duration _scrollFocusLifetime = Duration(250, MILLISECONDS);
        TimePoint _lastScrollEvent = TimePoint(0) - _scrollFocusLifetime;
    ...
    protected:
        ...
        EventTargets _OnMouseMove(int x, int y, int deltaX, int deltaY) override
        {
            ...
            if (_verticalScrollbar.held)
            {
                if (deltaY != 0)
                {
                    ...
                    ScrollPosition(Scrollbar::VERTICAL, (int)std::roundf(MaxScroll(Scrollbar::VERTICAL) * scrollRatio));
                }
            ...
            if (_horizontalScrollbar.held)
            {
                if (deltaX != 0)
                {
                    ...
                    float scrollRatio = scrolledLength / scrollableLength;
                    ScrollPosition(Scrollbar::HORIZONTAL, (int)std::roundf(MaxScroll(Scrollbar::HORIZONTAL) * scrollRatio));
                }
                return EventTargets().Add(this, GetMousePosX(), GetMousePosY());
            }
        ...
        EventTargets _OnLeftPressed(int x, int y) override
        {
            ...
            if (ScrollbarHovered(Scrollbar::VERTICAL) && _verticalScrollbar.backgroundVisible && _verticalScrollbar.interactable)
            {
                ...
                _verticalScrollbar.holdPos = int(y - ScrollbarHitbox(Scrollbar::VERTICAL).top);
            ...
            else if (ScrollbarHovered(Scrollbar::HORIZONTAL) && _horizontalScrollbar.backgroundVisible && _horizontalScrollbar.interactable)
            {
                ...
                _horizontalScrollbar.holdPos = int(x - ScrollbarHitbox(Scrollbar::HORIZONTAL).left);
                // Stop scroll animation
        ...
        {
            auto targets = EventTargets();
            if (ztime::Main() > _lastScrollEvent + _scrollFocusLifetime)
                targets = Panel::_OnWheelUp(x + _horizontalScrollbar.scrollAmount, y + _verticalScrollbar.scrollAmount);
        ...
        EventTargets _OnWheelDown(int x, int y) override
        {
            auto targets = EventTargets();
            if (ztime::Main() > _lastScrollEvent + _scrollFocusLifetime)
                targets = Panel::_OnWheelDown(x + _horizontalScrollbar.scrollAmount, y + _verticalScrollbar.scrollAmount);
